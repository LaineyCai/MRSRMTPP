...
def load_graph(data_dir):
    ...
    # loads graph: 'parentgraph.txt' 'siblinggraph.txt' 'selfgraph.txt' 'parent.pkl' 'sibling.pkl' 'self.pkl'
    parent_file = os.path.join(data_dir, 'graph.txt')
    parentpkl_file = os.path.join(data_dir, 'graph.pkl')

    if os.path.isfile(parentpkl_file):
        Gp = pickle.load(open(parentpkl_file, 'rb'))
    else:
        Gp = nx.Graph()
        Gp.name = data_dir+'graph'
...
def load_graph_withtrack(data_dir):
    ...
    if os.path.isfile(parentpkl_file):
        ...
        f.close()
        pickle.dump(Gp, open(parentpkl_file, 'wb'))
    return Gp, node_index, node_reverse_index
...
def convert_cascade_to_examples(sequence,
    ...
    for i, node in enumerate(sequence):
        ...
        for i_v, v in enumerate(prefix):
            ...
            #i_p = [node_pos[x] for x in dagp.predecessors(v)]
            '''
            for sip in i_p:
                #print(float(tssequence[i_v]) - float(tssequence[sip]))
                #if sip<i_v-3:
                if float(tssequence[i_v])-float(tssequence[sip])>1:
                    i_p.remove(sip)
            '''
            topop_mask[i_v, i_p] = 1
            #i_p = [node_pos[x] for x in dagt.predecessors(v)]
            i_p = []
    ...
    if os.path.isfile(pkl_path):
        ...
        with codecs.open(filename, 'r', encoding='utf-8') as input_file:
            for line_index, line in enumerate(input_file):
                ...
                sub_examples = convert_cascade_to_examples(sequence, tssequence, Gp=Gp)
