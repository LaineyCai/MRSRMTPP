...
def gru_layer(tparams, state_below, options, seq_masks=None, topop_masks=None, topot_masks=None):
    '''
    The GRU model.
    ...
    def _step(index, seq_m_, topo_mp_, topo_mt_, x_, h_arr_):
        '''
        A GRU step.
        ...
        beta_sum = beta_ap + beta_at
        # shape (n_samples, )
        alpha_ap = beta_ap / beta_sum
        alpha_at = beta_at / beta_sum

        h_sum = alpha_ap[:,None] * h_sump + alpha_at[:, None]*h_sumt
...
def build_model(tparams, timetparams, options):
    ...
    inputs = [seqs, seq_masks, topop_masks, topot_masks]
    ...
    h_arr = gru_layer(tparams, embs, options, seq_masks=seq_masks, topop_masks=topop_masks, topot_masks=topot_masks)
    ...
    #shape (n_sample,1)
    tss = tensor.abs_((tensor.dot(h_mean, timetparams['W_t']) * tg_masks).sum(axis=1, keepdims=True) + tensor.dot(tg_masks,timetparams['b_t']))
