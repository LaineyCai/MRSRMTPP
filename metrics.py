...
Author: Hongyun Cai
Date: 2018.3.13
Functionality: Evaluation metrics functions.
'''
# import math
import numpy as np
import collections
...
def rmse(yt_prob, yt, tdim):
    #mse = np.square(yt_prob - yt)[yt!=0]
    mse = np.square(yt_prob - yt)
    return tdim*math.sqrt(np.mean(mse))
...
def portfolio(y_prob, y, yt_prob, yt, tdim, k_list=None):
    y_prob, y = _retype(y_prob, y)
    yt_prob, yt = _retype(yt_prob, yt)
    ...
    scores['rmse'] = rmse(yt_prob, yt, tdim)
